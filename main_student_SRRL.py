'''
    main function to train the student network (SRRL)
'''
import torch
from utils import load_training_set, load_dataset, get_dataloader, draw_curve, func_GTSave, path_exists_make, detect_changes_batch_SRRL
from model import StudentVGGNet, VGGNet, ResNet50, ResNet18, SRRL, shufflenetv2, MobileNetv2
from train import train_student_SRRL, test_student_SRRL
import numpy as np
...
if pca:
    if model_name == 'resnet':
        model_stu = ResNet18(bands=numComponents, mode=KD_mode, srrl=srrl).to(device)
        model_tea = ResNet50(bands=numComponents, mode=KD_mode, srrl=srrl).to(device)
    elif model_name == 'vgg':
        model_stu = StudentVGGNet(bands=numComponents, mode=KD_mode, srrl=srrl).to(device)
        model_tea = VGGNet(bands=numComponents, mode=KD_mode, srrl=srrl).to(device)
else:
    if model_name == 'resnet':
        model_stu = ResNet18(bands=bands, mode=KD_mode, srrl=srrl).to(device)
        model_tea = ResNet50(bands=bands, mode=KD_mode, srrl=srrl).to(device)
    elif model_name == 'vgg':
        model_stu = StudentVGGNet(bands=bands, mode=KD_mode, srrl=srrl).to(device)
...
# print(feat_t.shape) #([2, 2048, 3, 3])
# print(feat_s.shape) #([2, 4608])
# time.sleep(1000)
...
for t in range(epochs):
    print(f"Epoch {t + 1}\n-------------------------------")
    train_loss, train_acc = train_student_SRRL(train_loader, module_list, criterion_list, optimizer, device, KD_mode, model_name)
    test_loss, test_acc = test_student_SRRL(test_loader, module_list, criterion, device, KD_mode, model_name)
    stepLR.step()
    ...
    oa, kc, mask = detect_changes_batch_SRRL(module_list, data_path, hsi_gt_b, device, batch_size, KD_mode, model_name)
    sio.savemat(os.path.join(result_path, 'mask_epoch_{}_oa_{}_kappa_{}.mat'.format(t, oa, kc)), {'mask': mask})
    torch.save(model_stu.state_dict(), os.path.join(os.path.join(os.path.join(os.path.join('SRRL\\model', dataset), model_name), 'student'), 'model_epoch_{}_oa_{}_kappa_{}.pth'.format(t, oa, kc)))
